...
function habit(repeatDays: number, recurring = true, excludeHolidays = false): Item {
  return {
    id: "habit-1",
    title: "テスト習慣",
    tag: null,
...
describe("曜日のくりかえし", () => {
  ...
  it("祝日除外を選ぶと祝日・振替休日・国民の休日を除外する", () => {
    const item = habit(WEEKDAY_REPEAT_DAYS, true, true);
  ...
  it("毎日を選んだままでも祝日だけを除外できる", () => {
    const item = habit(ALL_REPEAT_DAYS, true, true);
    expect(isHabitScheduledOn(item, "2026-07-19")).toBe(true); // 通常の日曜
    expect(isHabitScheduledOn(item, "2026-07-20")).toBe(false); // 海の日
  });

  it("祝日除外を選ばなければ個別に選んだ曜日は祝日でも表示する", () => {
  ...
  it("曜日の表示名を短く整形する", () => {
    ...
    expect(formatRepeatDays(WEEKDAY_REPEAT_DAYS)).toBe("月・火・水・木・金");
    expect(formatRepeatDays(ALL_REPEAT_DAYS, true)).toBe("毎日（祝日除く）");
